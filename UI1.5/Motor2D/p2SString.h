...
public:
	...
	bool Cut(unsigned int begin, unsigned int end = 0)
	{
		...
		//modif to cut the last char
		/*
		if (begin == len && end == 0 && (Length() > 0))
		{
		str[len - 1] = '\0';
		return true;
		}
		*/
		if (begin == (len - 1) && end == len && (Length() > 0))
		{
			str[len - 1] = '\0';
			return true;
		}
	...
	const char* ModifyStrToGetStrFromStartToPosition(int position)
	{
		if (str[0] == '\0')
		{
			return GetString();
	...
	bool Insert(int position, p2SString new_buffer)
	{
		uint len = Length();
